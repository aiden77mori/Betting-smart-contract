...
contract("SmartBet", async (accounts) => {
  ...
  context("when create match is called", function () {
    it("should only allow the contract owner create a match", async () => {
      await catchCustomError(
        createTestMatch(1, 1, 2, 3, bettor),
        "caller is not owner"
      );
    ...
    it("should create matches with correct details", async () => {
      ...
      let result = await createTestMatch(
        apiMatchId,
        oddsTeamA,
        oddsTeamB,
        oddsDraw,
      ...
      expect(match["matchResultLink"]).to.equals(
        matchResultLink,
        "incorrect match resultLink"
      );
      assert.equal(
        match["totalPayoutTeamA"],
        0,
        "incorrect match totalPayoutTeamA"
      );
      assert.equal(
        match["totalPayoutTeamB"],
        0,
        "incorrect match totalPayoutTeamB"
      );
      assert.equal(
        match["totalPayoutDraw"],
        0,
        "incorrect match totalPayoutDraw"
      );
      assert.equal(
        match["totalCollected"],
        0,
        "incorrect match totalCollected"
      );
      assert.equal(
        match["result"],
        SmartBet.enums.MatchResult.NOT_DETERMINED,
        "incorrect match result"
      );
      assert.equal(
        match["state"],
        SmartBet.enums.MatchState.NOT_STARTED,
    ...
    it("should emit MatchCreatedEvent when match is created successfully", async () => {
      ...
      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "MatchCreatedEvent"
      );
      assert.isNotEmpty(eventLogs, "MatchCreatedEvent was not emitted");
      assert.lengthOf(
        eventLogs,
        1,
        "More than one MatchCreatedEvent was emitted"
      );
  ...
  context("when place bet is called", function () {
    it("should only allow bets on match that exist", async () => {
      await catchCustomError(
        smartBetInstance.placeBet(1, SmartBet.enums.MatchResult.TEAM_A_WON),
    ...
    it("should only allow bets on match that have not started", async () => {
      ...
      await catchCustomError(
        smartBetInstance.placeBet(1, SmartBet.enums.MatchResult.TEAM_A_WON, {
          from: bettor,
          value: generateValue(1),
        }),
        "match started"
      );

      await smartBetInstance.createMatch(
        2,
        MATCH_RESULT_API + 2,
        1,
        2,
      ...
      await smartBetInstance.placeBet(
        2,
        SmartBet.enums.MatchResult.TEAM_A_WON,
        { from: bettor, value: generateValue(1) }
      );
      await catchCustomError(
        smartBetInstance.placeBet(2, SmartBet.enums.MatchResult.TEAM_A_WON, {
          from: bettor,
          value: generateValue(1),
        }),
        "match started"
    ...
    it("should update the total payout on the match result outcome that is bet on", async () => {
      ...
      let totalExpectedPayout = bettor1Stake
        .mul(new BN(oddsTeamA))
        .add(bettor2Stake.mul(new BN(oddsTeamA)))
        .div(new BN(100));
      ...
      expect(match["totalPayoutTeamA"]).to.equal(
        totalExpectedPayout.toString(),
        "Incorrect totalPayoutTeamA value"
      );

      bettor1Stake = generateValue(2);
      bettor2Stake = generateValue(2);
      totalExpectedPayout = bettor1Stake
        .mul(new BN(oddsTeamB))
        .add(bettor2Stake.mul(new BN(oddsTeamB)))
        .div(new BN(100));
      await smartBetInstance.placeBet(
        1,
        SmartBet.enums.MatchResult.TEAM_B_WON,
        { from: bettor, value: bettor1Stake }
      );
      await smartBetInstance.placeBet(
        1,
      ...
      expect(match["totalPayoutTeamB"]).to.equal(
        totalExpectedPayout.toString(),
        "Incorrect totalPayoutTeamB value"
      );
      ...
      totalExpectedPayout = bettor1Stake
        .mul(new BN(oddsDraw))
        .add(bettor2Stake.mul(new BN(oddsDraw)))
        .div(new BN(100));
      ...
      match = await smartBetInstance.getMatch(1);
      expect(match["totalPayoutDraw"]).to.equal(
        totalExpectedPayout.toString(),
        "Incorrect totalPayoutDraw value"
      );
    });
    ...
    it("should update the total amount collected on the match", async () => {
      let bettor1Stake = generateValue(1);
      let bettor2Stake = generateValue(2);
      let totalExpectedCollected = bettor1Stake
        .mul(new BN(2))
        .add(bettor2Stake.mul(new BN(3)));
      ...
      let match = await smartBetInstance.getMatch(1);
      expect(match["totalCollected"]).to.equal(
        totalExpectedCollected.toString(),
        "Incorrect totalCollected value"
      );
    });
    ...
    it("should issue asset with correct values when bet is placed", async () => {
      let oddsTeamA = 4;
      let bettor1Stake = generateValue(1);
      let expectedAssetValue = bettor1Stake
        .mul(new BN(oddsTeamA))
        .div(new BN(100));
      ...
      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "SmartAssetAwardedEvent"
      );
      assert.isNotEmpty(eventLogs, "SmartAssetAwardedEvent was not emitted");
      assert.lengthOf(
        eventLogs,
        1,
        "More than one SmartAssetAwardedEvent was emitted"
      );
      expect(eventLogs[0].args["awardee"]).to.equal(
        bettor,
        "Incorrect smart asset awardee"
      );

      let smartAssetId = eventLogs[0].args["smartAssetId"];
      let smartAsset = await smartBetInstance.getSmartAsset(smartAssetId, {
        from: bettor,
      });
      expect(smartAsset["owner"]).to.equal(bettor, "Incorrect asset owner");
      expect(smartAsset["matchId"]).to.equal(
        "1",
        "Incorrect match id on smart asset"
      );
      expect(smartAsset["accruedInterest"]).to.equal(
        "0",
        "Incorrect accured interest"
      );
      assert.equal(
        smartAsset["matchResult"],
        resultBetOn,
        "Incorrect match result on smart asset"
      );
      expect(smartAsset["initialValue"]).to.equal(
        expectedAssetValue.toString(),
        "Incorrect asset value"
      );
    });
    ...
    it("should emit BetPlacedEvent when bet is placed successfully", async () => {
      ...
      await createTestMatch(1);
      let result = await smartBetInstance.placeBet(
        1,
        SmartBet.enums.MatchResult.TEAM_A_WON,
        { from: bettor, value: bettor1Stake }
      );
      ...
      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "BetPlacedEvent"
      );
      ...
      expect(eventLogs[0].args["matchId"].toString()).to.equal(
        "1",
        "Incorrect match ID"
      );
      expect(eventLogs[0].args["amount"].toString()).to.equal(
        bettor1Stake.toString(),
        "Incorrect bet placed amount"
      );
    });
  });
  ...
  context("when start match is called", function () {
    it("should only allow contract owner start a match", async () => {
      await createTestMatch();
      await catchCustomError(
        smartBetInstance.startMatch(1, { from: bettor }),
        "caller is not owner"
      );
    });
    ...
    it("should only allow match that has not started to be started", async () => {
      await createTestMatch();
      await smartBetInstance.startMatch(1);
      await catchCustomError(smartBetInstance.startMatch(1), "match started");
    });
  ...
  context("when close match is called", function () {
    it("should only allow contract owner close a match", async () => {
      ...
      await smartBetInstance.startMatch(1);
      await catchCustomError(
        smartBetInstance.closeMatch(1, SmartBet.enums.MatchResult.TEAM_A_WON, {
          from: bettor,
        }),
        "caller is not owner"
      );
    ...
    it("should update the state of the match to finished when closed", async () => {
      await createTestMatch();
      await smartBetInstance.startMatch(1);
      await smartBetInstance.closeMatch(
        1,
        SmartBet.enums.MatchResult.TEAM_A_WON
      );
      ...
      assert.equal(
        match["state"],
        SmartBet.enums.MatchState.FINISHED,
        "Incorrect match state"
      );
    });
    ...
    it("should emit MatchResultSetEvent when match is closed successfully", async () => {
      await createTestMatch();
      await smartBetInstance.startMatch(1);
      let result = await smartBetInstance.closeMatch(
        1,
        SmartBet.enums.MatchResult.TEAM_B_WON
      );

      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "MatchResultSetEvent"
      );
      assert.isNotEmpty(eventLogs, "MatchResultSetEvent was not emitted");
      assert.lengthOf(
        eventLogs,
        1,
        "More than one MatchResultSetEvent was emitted"
      );
      assert.equal(
        eventLogs[0].args["result"].toNumber(),
        SmartBet.enums.MatchResult.TEAM_B_WON,
        "Incorrect match result"
      );
      expect(eventLogs[0].args["matchId"].toString()).to.equal(
    ...
    it("should only invalidate assets of match losers when a match is closed", async () => {
      ...
      await smartBetInstance.getSmartAsset(bettor1SmartAssetId, {
        from: bettor,
      });

      //bettor2 place bet
      result = await smartBetInstance.placeBet(
        1,
        SmartBet.enums.MatchResult.DRAW,
        { from: bettor2, value: bettor2Stake }
      );

      let bettor2SmartAssetId = getSmartAssetIdFromResult(result);
      await smartBetInstance.getSmartAsset(bettor2SmartAssetId, {
        from: bettor2,
      });
      ...
      await catchCustomError(
        smartBetInstance.getSmartAsset(bettor1SmartAssetId, { from: bettor }),
        "Invalid token"
      );
      await smartBetInstance.getSmartAsset(bettor2SmartAssetId, {
        from: bettor2,
    ...
    it("should emit MatchClosedEvent when match is closed successfully", async () => {
      ...
      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "MatchClosedEvent"
      );
      assert.isNotEmpty(eventLogs, "MatchClosedEvent was not emitted");
      assert.lengthOf(
        eventLogs,
        1,
        "More than one MatchClosedEvent was emitted"
      );
      expect(eventLogs[0].args["by"]).to.equal(owner);
      expect(eventLogs[0].args["matchId"].toNumber()).to.equal(
        1,
        "Incorrect match ID"
      );
    });
  ...
  context("when liquidate asset is called", function () {
    it("should not be allowed to liquidate asset if circuit breaker is on", async () => {
      ...
      await smartBetInstance.toggleCircuitBreaker();
      await catchCustomError(
        smartBetInstance.liquidateAsset(bettor1SmartAssetId, { from: bettor }),
        "Circuit breaker is on"
      );

      await smartBetInstance.toggleCircuitBreaker();
      await smartBetInstance.liquidateAsset(bettor1SmartAssetId, {
        from: bettor,
      });
    });
    ...
    it("should only allow valid assets to liquidated", async () => {
      ...
      await catchCustomError(
        smartBetInstance.liquidateAsset(bettor1SmartAssetId.add(new BN(1)), {
          from: bettor,
        }),
        "Invalid token"
      );
    ...
    it("should only allow asset owner to liquidate asset", async () => {
      ...
      await catchCustomError(
        smartBetInstance.liquidateAsset(bettor1SmartAssetId, { from: bettor2 }),
    ...
    it("should only allow assets tied to finished matches to be liquidated", async () => {
      ...
      await catchCustomError(
        smartBetInstance.liquidateAsset(bettor1SmartAsset1Id, { from: bettor }),
        "Cannot liquidate asset until match is finished"
      );
      await smartBetInstance.liquidateAsset(bettor1SmartAsset2Id, {
        from: bettor,
      });
    });
    ...
    it("should transfer correct asset value to asset owner's address", async () => {
      ...
      let bettor1SmartAssetId = getSmartAssetIdFromResult(result);
      let smartAsset = await smartBetInstance.getSmartAsset(
        bettor1SmartAssetId,
        { from: bettor }
      );
      ...
      result = await smartBetInstance.liquidateAsset(bettor1SmartAssetId, {
        from: bettor,
      });
      let gasUsed = getGasUsedFromResult(result);
      let gasPrice = await web3.eth.getGasPrice();
      let totalGasCost = new BN(gasUsed).mul(new BN(gasPrice));
      let expectedBalanceAfter = new BN(bettor1BalanceBefore)
        .add(new BN(smartAssetValue))
        .sub(totalGasCost);

      let bettor1BalanceAfter = await web3.eth.getBalance(bettor);

      expect(bettor1BalanceAfter).to.equal(
        expectedBalanceAfter.toString(),
        "Incorrect bettor's balance after asset liquidation"
      );
    });
    ...
    it("should emit AssetLiquidatedEvent after asset is liquidated successfully", async () => {
      ...
      let eventLogs = result.logs.filter(
        (log) => log.event && log.event == "AssetLiquidatedEvent"
      );
      assert.isNotEmpty(eventLogs, "AssetLiquidatedEvent was not emitted");
      assert.lengthOf(
        eventLogs,
        1,
        "More than one AssetLiquidatedEvent was emitted"
      );
  ...
  context("when toogle circuit breaker is called", function () {
    it("should only allow contract owner toogle circuit breaker", async () => {
      await catchCustomError(
        smartBetInstance.toggleCircuitBreaker({ from: bettor }),
        "caller is not owner"
      );
    });
  });
  ...
  context("when get match is called", function () {
    it("should throw error if match ID does not exist", async () => {
      ...
      await catchCustomError(
        smartBetInstance.getMatch(2, { from: bettor }),
        "on chain match does not exist"
      );
    });
  });
  ...
  context("when get smart asset is called", function () {
    it("should throw error if asset is invalid", async () => {
      ...
      await catchCustomError(
        smartBetInstance.getSmartAsset(bettor1SmartAssetId.add(new BN(1)), {
          from: bettor,
        }),
        "Invalid token"
      );
    ...
    it("should only allow asset owner view asset details", async () => {
      ...
      await catchCustomError(
        smartBetInstance.getSmartAsset(bettor1SmartAssetId, { from: bettor2 }),
        "caller is not token owner"
      );
    });
  ...
  context("when change owner is called", function () {
    it("should only allow owner change owner", async () => {
      await catchCustomError(
        smartBetInstance.changeOwner(accounts[1], { from: accounts[2] }),
        "caller is not owner"
      );
    });
  ...
  async function createTestMatch(
    ...
    return await smartBetInstance.createMatch(
      apiId ? apiId : 1,
      matchResultLink ? matchResultLink : MATCH_RESULT_API + apiId,
      oddsTeamA ? oddsTeamA : 1,
      oddsTeamB ? oddsTeamB : 2,
      oddsDraw ? oddsDraw : 3,
      Math.floor((Date.now() + 60000) / 1000),
      { from: creator ? creator : owner }
    );
  }
